...
static int _modbus_tcp_connect(modbus_t *ctx)
{
	...
	rc = _connect(ctx->s, (struct sockaddr*) &addr, sizeof(addr),
...
static int _modbus_tcp_pi_connect(modbus_t *ctx)
{
	...
	for (ai_ptr = ai_list; ai_ptr != NULL; ai_ptr = ai_ptr->ai_next)
	{
		...
		rc = _connect(s, ai_ptr->ai_addr, ai_ptr->ai_addrlen,
				&ctx->response_timeout);
		if (rc == -1)
		{
			close(s);
...
int modbus_tcp_listen(modbus_t *ctx, int nb_connection)
{
	...
	enable = 1;
	if (setsockopt(new_s, SOL_SOCKET, SO_REUSEADDR, (char*) &enable,
			sizeof(enable)) == -1)
	{
		close(new_s);
		return -1;
...
int modbus_tcp_pi_listen(modbus_t *ctx, int nb_connection)
{
	...
	for (ai_ptr = ai_list; ai_ptr != NULL; ai_ptr = ai_ptr->ai_next)
	{
		...
		{
			int enable = 1;
			rc = setsockopt(s, SOL_SOCKET, SO_REUSEADDR, (void*) &enable,
					sizeof(enable));
			if (rc != 0)
			{
				close(s);
